...
//    private WebView webB;
    ...
    public webPage2()
	{
        ...
        if (lstgyouretu.GetList(wurl, ref srtErrMsg) == true)
        {
            ...
            int ret_id1 = 0;
            int ret_id2 = 0;
            //HorizontalStackLayout w_layoutgtiy;
            for (i_gyou = 0; i_gyou <= lstgyouretu._Header._Retu; i_gyou++)
            {
                HorizontalStackLayout w_layoutgtiy = new HorizontalStackLayout()
                {
                ...
                ret_id1 = lstgyouretu.FindData(i_gyou, i_retu);
                if (ret_id1 != -1)
                {
                    //行を作って行データのを表示


                    //Label wlabel1 = MakeNewDataLabelTextColorParts(ret_id1);
                    Label wlabel1 = new Label
                    {
                        Text = lstgyouretu._Datas[ret_id1]._visible == true ? lstgyouretu._Datas[ret_id1]._text : "　　",
                        BackgroundColor = Colors.White,
                        TextColor = GetgyouretuColor(lstgyouretu._Datas[ret_id1]._fontcolor),
                        FontSize = lstgyouretu._Datas[ret_id1]._fontsize,
                        FontAttributes = lstgyouretu._Datas[ret_id1]._fontBOLD ? FontAttributes.Bold : FontAttributes.None,
                        WidthRequest = lstgyouretu._Datas[ret_id1]._width,
                        VerticalOptions = LayoutOptions.Center,
                        //            HorizontalOptions = LayoutOptions.Fill,
                        HorizontalOptions = LayoutOptions.Center,
                    ...
                    w_layoutgtiy.Children.Add(wlabel1);
                ...
                for (i_retu = 0; i_retu <= lstgyouretu._Header._Retu; i_retu++)
                {
                    ret_id2 = lstgyouretu.FindData(i_gyou, i_retu);
                    if (ret_id2 != -1)
                    {
                        //行を作って行データのを表示
                        //Label wlabel2 = MakeNewDataLabelTextColorParts(ret_id2);
                        Label wlabel2 = new Label
                        {
                            Text = lstgyouretu._Datas[ret_id2]._visible == true ? lstgyouretu._Datas[ret_id2]._text : "　　",
                            BackgroundColor = Colors.White,
                            TextColor = GetgyouretuColor(lstgyouretu._Datas[ret_id2]._fontcolor),
                            FontSize = lstgyouretu._Datas[ret_id2]._fontsize,
                            FontAttributes = lstgyouretu._Datas[ret_id2]._fontBOLD ? FontAttributes.Bold : FontAttributes.None,
                            WidthRequest = lstgyouretu._Datas[ret_id2]._width,
                            VerticalOptions = LayoutOptions.Center,
                            //            HorizontalOptions = LayoutOptions.Fill,
                            HorizontalOptions = LayoutOptions.Center,
                        ...
                        w_layoutgtiy.Children.Add(wlabel2);
                    }
                    else
                    {
                    int i = 0;
                ...
                layout1.Children.Add(w_layoutgtiy);
            }
        }
