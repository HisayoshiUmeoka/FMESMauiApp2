...
namespace FMES
{
    class clsKaisouList
    {
        ...
        public List<clsKaisou> _Datas = new List<clsKaisou>();
            ...
            if (string.IsNullOrEmpty(strRet) == false)
            {
                if (strRet.IndexOf("NG:") == -1)
                {
                    ...
                    foreach (string wStr in lstRec)
                    {
                        ...
                        {
                            clsKaisou wKaisou = new clsKaisou(wStr);
                            _Datas.Add(wKaisou);
                        }
                        iRow += 1;
                    }
        ...
        public bool GetList(int wUserID, string wSasizuNo, int wKaisouNo, int wKouteiID, int wKouteiShousaiID, int wKensaBui, int wKensaBashoShousaiID)
        {
            bool bRet = false;
            int iRow = 0;
            string wURL = clsGlobalVar.GetCurURL() + "users/tabworkprocesssearch/" + clsGlobalVar.GetLanguageStr() + "/" + wUserID + "/" + wSasizuNo + "/" + wKaisouNo + "/" + wKouteiID + "/" + wKouteiShousaiID + "/" + wKensaBui + "/" + wKensaBashoShousaiID;
            string strErr = string.Empty;
            SendLog(wURL, ref strErr);
            string strRet = clsWebUpDown.GetWebResponce(wURL);
            if (string.IsNullOrEmpty(strRet) == false)
