...
namespace FMES
{
    class clsVerList
    {
        ...
        public List<clsVerData> _Datas = new List<clsVerData>();
        public bool GetList(int wUserID, string wSasizuNo, ref string srtErrMsg)
        {
            bool bRet = false;
            int iRow = 0;
            _SasizuNo = wSasizuNo;
            string wURL = clsGlobalVar.GetCurURL() + "users/tabprocessversion/" + clsGlobalVar.GetLanguageStr() + "/" + wUserID + "/" + wSasizuNo;
            ...
            if (string.IsNullOrEmpty(strRet) == false)
            {
                if (strRet.IndexOf("NG:") == -1)
                {
                    ...
                    {
                        clsVerData wVerData = new clsVerData(wStr);
                        _Datas.Add(wVerData);
                        iRow += 1;
                    }
                    lstRec.Clear();
